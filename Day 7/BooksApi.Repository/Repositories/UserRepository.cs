...
namespace BooksApi.Repository.Repositories
{
    public class UserRepository(BookDbContext bookDbContext) : IUserRepository
    {
        ...
        {
            var query = _dbContext.Users.AsQueryable();//Asqueryable is used to generate query
            ...
            if (!string.IsNullOrWhiteSpace(model.SortBy) && model.SortBy == "Name")//sort by name
            {
            ...
            else if (!string.IsNullOrWhiteSpace(model.SortBy) && model.SortBy == "Id")//sort by id
            {
                if (model.SortDirection == "asc")
            ...
            query = query.Skip(model.PageNumber * model.PageSize).Take(model.PageSize);//pagesize
            ...
            var dataQuery = query.Include(u => u.BookDetails).Select(u => new UserModel()
            {
                ...
                Id = u.Id,
                Role = u.Role,
                BookDetails = u.BookDetails.ToList()//tolist is used to joining through LINQ
            });
